...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	int Status = XSpi_Transfer(&SpiInstance, Buffer, NULL, 2);
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int Status = XSpi_Transfer(&SpiInstance, Buffer, NULL, (nbytes + 1));
...
BYTE MAXreg_rd(BYTE reg) {
    ...
    int Status = XSpi_Transfer(&SpiInstance, write, readBuf, 2);
...
//	}
    ...
    int Status = XSpi_Transfer(&SpiInstance, sendBuf, recBuf, nbytes+1);
